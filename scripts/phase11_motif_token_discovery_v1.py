...
def run_seed(
    ...
    def report_cb(state, step, ep_ledger, accepted_frac):
        ...
        maps_dict = compute_spatial_maps(state, ["sigma", "k_axis_bias", "k_entropy", "mismatch"])
        ...
        for feat in motif_features:
            name, arr = _select_map(maps_dict, feat, motif_interface, 0)
        ...
        if snapshot_every > 0 and window_idx % snapshot_every == 0:
            ...
            for feat in ["k_axis_bias", "k_entropy", "mismatch", "sigma"]:
    ...
    jsd_hazard_post = jsd(hazard_counts, post_counts) if post_counts.sum() > 0 else 0.0
    jsd_pre_post = jsd(baseline_counts, post_counts) if post_counts.sum() > 0 else 0.0
    ...
    result = {
        ...
        "jsd_hazard_post": jsd_hazard_post,
        "jsd_pre_post": jsd_pre_post,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase 11 motif/token discovery")
    ...
    report_path = out_dir / "PHASE11_MOTIF_REPORT.md"
    with report_path.open("w", encoding="utf-8") as fh:
        fh.write("# Phase 11 motif/token discovery v1\n\n")
        fh.write(
            "| seed | status | coverage_pre | coverage_hazard | jsd_pre_hazard | prop_score | pass |\n"
        )
        fh.write("| ---: | --- | ---: | ---: | ---: | ---: | --- |\n")
        for row in rows:
            fh.write(
                f"| {row['seed']} | {row['status']} | {row['coverage_baseline']:.6g} | "
                f"{row['coverage_hazard']:.6g} | {row['jsd_pre_hazard']:.6g} | "
